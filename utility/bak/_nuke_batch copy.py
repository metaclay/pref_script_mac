import nuke
import os
import shutil
import sys
import clayproject
import cxsetting

FILE_LIST = os.path.expanduser("~/Documents/preferences/script/utility/_file_list.txt")

# NEW: Tracking lists
FAILED = []      # (path, reason)
SKIPPED = []     # (path, reason)
# Mode descriptions mapping for better output

MODE = 4  # Set desired mode here (1, 2, 3, 4, or 5)
REPLACE_RENDER_GROUP = 1  # 1 = replace existing render group, 0 = skip if render group exists
RENDER_VERSION = 0 # 0-render v0 , 1-render Final.
NODES_TO_DELETE = [
    "Precomp1"
    ]
    # Add more node names to delete if needed]

_NODES_V0_ = clayproject._NODES_V0_
_NODES_OUTPUT_ = clayproject._NODES_OUTPUT_
_RENDER_V0_ = clayproject._RENDER_V0_
_RENDER_FINAL_ = clayproject._RENDER_FINAL_

RENAME_MAP = {
    "_V0_NODES": "_NODES_V0_",
}

MODE_DESCRIPTIONS = {
    1: "INTIAL SETUP - CLAYPROJECT NODES CLEANUP",
    2: "CREATE RENDER GROUP" + (" > RENDER V000" if RENDER_VERSION==0 else " > RENDER FINAL"),
    3: "RENDER " + ("(Non-Modifying)" if REPLACE_RENDER_GROUP ==0 else "") + (" > RENDER V000" if RENDER_VERSION==0 else " > RENDER FINAL") ,
    4: "DELETE NODES",
    5: "RESET ROOT CX",
    6: "RENAME NODES",
}

MAX_FILE_LIST=6

# ----------------------------------------------------
# 📂 CORE FILE I/O AND UTILITY FUNCTIONS
# ----------------------------------------------------

def read_file_list(path):
    print("\n[READ FILE LIST] Loading:", path)
    files = []

    if not os.path.exists(path):
        print("  ❌ FILE LIST NOT FOUND:", path)
        return files

    try:
        seen = set()
        with open(path, "r") as f:
            for line in f:
                clean = line.strip()
                if clean and clean not in seen:
                    files.append(clean)
                    seen.add(clean)

        print(f"  ✔ Loaded {len(files)} unique file entries.")

    except Exception as e:
        print(f"  ❌ Error reading list: {e}")

    return files


def backup_original(nk_path):
    """
    Creates a backup of the NK file in an 'archives' subfolder.
    Adds sequential numbering (e.g., -01, -02) if the file already exists.
    """
    print("  --->  Backing up original NK file ...")

    nk_dir = os.path.dirname(nk_path)
    nk_file = os.path.basename(nk_path)
    name, ext = os.path.splitext(nk_file)

    archive_dir = os.path.join(nk_dir, "archives")
    
    # Sequential Naming Logic
    archive_name = nk_file
    counter = 0
    
    while True:
        if counter == 0:
            archive_path = os.path.join(archive_dir, archive_name)
        else:
            # Format the sequential number with leading zero (e.g., -01, -02)
            archive_name = f"{name}-{counter:02d}{ext}"
            archive_path = os.path.join(archive_dir, archive_name)
        
        if not os.path.exists(archive_path):
            break
            
        counter += 1

    try:
        os.makedirs(archive_dir, exist_ok=True)
        shutil.copy2(nk_path, archive_path)
        print(f"  ✅ Backup saved to: **{archive_path}**")
    except Exception as e:
        print(f"  ❌ Failed to backup file: {e}")
        FAILED.append((nk_path, f"Backup failed: {e}"))


# ----------------------------------------------------
# ⚙️ PROCEDURE TEMPLATE FUNCTIONS (MODE 1, 2, 3, 4)
# ----------------------------------------------------

# MODE 1: Initial Setup
#------------------------------
def initial_setup(nk_path, ignore_skipped=0, ignore_failed=0):
    # """
    # For new projects - freshed exported from Nuke Studio.
    # * Deletes 'Precomp1' node if it exists.
    # * Runs clayproject.nodes_cleanup()
    # * Sets up root CX using cxsetting.cxsetting()
    # """

    # # print("  --->  Checking for '__cx__' node ...")

    # # cx_node = nuke.toNode("__cx__")

    # # if not cx_node:
    # #     msg = "__cx__ not found → skipping nodes_cleanup"
    # #     print("  ❌", msg)
    # #     if not ignore_skipped :
    # #         SKIPPED.append((nk_path, msg))
    # #     return False

    # # print("  ✔ __cx__ found - proceeding with nodes_cleanup ...")

    # try:
    #     print()
    #     print(" DELETING Precomp1 IF EXISTS ... ")
    #     if nuke.toNode("Precomp1") :
    #         nuke.delete(nuke.toNode("Precomp1"))
    #         print("  ✅ Precomp1 deleted.")
    #     else :
    #         print("  ▶ Precomp1 not found, skipping deletion.")

    #     # print()
    #     # print(" CLEANING UP NODES")
    #     # clayproject.nodes_cleanup()
    #     # print("  ✅ nodes_cleanup completed successfully.")

    #     print()
    #     print(" SETUP ROOT CX")
        
    #     f = cxsetting.cxsetting()
    #     g = f()
    #     g()
        
    #     print("  ✅ Root CX created successfully.")

    #     return True # File was modified

    # except Exception as e:
    #     msg = f"nodes_cleanup failed: {e}"
    #     print("  ❌", msg)
    #     if not ignore_failed :
    #         FAILED.append((nk_path, msg))
    #     return False

    None


# MODE 2: Create Render Group
#------------------------------
def create_render_group(nk_path, replace_render_group=0 ,  ignore_skipped=0, ignore_failed=0, render_version=0):
    print("  --->  Running create_render_group procedure ...")

    # First, run node cleanup to ensure a clean state
    print("  ▶ Running node cleanup before creating render group ...   ")
    initial_setup(nk_path, ignore_skipped=1, ignore_failed=1)
    print("  ▶ Node cleanup completed. Proceeding with render group creation...")
    print()
    print("  ▶ Reset CX start ... ")
    nuke.root().knob('cxallreset').execute() # reset CX
    print("  ▶ Reset CX done")

    if render_version==0 :
        render_group_node = _NODES_V0_
    else :
        render_group_node = _NODES_OUTPUT_
        

    # 1. Check for render group node
    render_group_list = any(n.name().startswith(render_group_node) for n in nuke.allNodes())
    
    if render_group_list: 
        msg = "Node " + render_group_node + " found "+ ("→ Skipping render group creation." if not replace_render_group else "→ OVERWRITING existing render group.")
        print("  ⚠️", msg)
        if not replace_render_group:
            # Note: We treat this as "no change" to the file, 
            # so it doesn't trigger a backup/save.
            if not ignore_skipped :
                SKIPPED.append((nk_path, msg))
            return False

    else:
        print("  ✔ Render group not found. Proceeding with render group creation.")
    
    try:
        # 2. Import and run the render group creation function
        clayproject.clay_shortcut_render(replace=replace_render_group, render_version=RENDER_VERSION) 

        print("  ✅ Render group creation is completed successfully.")
        # Return True to signal that the script has been modified and needs saving
        return True 
        
    except ImportError:
        msg = "Render group creation failed: Required module 'cxdef' not found."
        print("  ❌", msg)
        if not ignore_failed :
            FAILED.append((nk_path, msg))
        return False
        
    except Exception as e:
        msg = f"Render group creation failed: {e}"
        print("  ❌", msg)
        if not ignore_failed :
            FAILED.append((nk_path, msg))
        return False


# MODE 3: Render
#------------------------------
def render(nk_path, render_version=0):

    print("  --->  Running render procedure  ...")
    
    if render_version == 0 :
        render_node_name = _RENDER_V0_
    else :
        render_node_name = _RENDER_FINAL_

    # 1. Check for the specific render node
    render_node = nuke.toNode(render_node_name)
    
    # 2. If the render node doesn't exist, try to create the new render group
    render_group_created = False
    check = False
    if not render_node:
        print("  ▶ Render node '" + render_node_name + "' not found. Running create_render_group() setup...")
        render_group_created = create_render_group(nk_path, replace_render_group=REPLACE_RENDER_GROUP ,ignore_skipped=1, ignore_failed=1, render_version=render_version ) 
        check = True

    else :
        if REPLACE_RENDER_GROUP :
            print("  ✔ Render node '" + render_node_name + "' found. Replacing with new one !!!")
            render_group_created = create_render_group(nk_path, replace_render_group=REPLACE_RENDER_GROUP ,ignore_skipped=1, ignore_failed=1 , render_version=render_version) 
            check = True

        else :
            print("  ✔ Render node '" + render_node_name + "' found. Starting render.")


    if check :
        if not render_group_created:
            print("  ❌ create_render_group failed or was skipped. Cannot proceed with render.")
            # If create_render_group failed, it already appended to FAILED.
            return False 

        # Now that create_render_group has run, check for the render node again
        render_node = nuke.toNode(render_node_name)
        
        if not render_node:
            msg = "Render node '" + render_node_name + "' still not found after running create_render_group(). Aborting render."
            print("  ❌", msg)
            FAILED.append((nk_path, msg))
            return False           
    
    
    try:
        # Get frame range from the script
        first = int(nuke.root().firstFrame())
        last = int(nuke.root().lastFrame())

        # 3. Execute the render node
        nuke.execute(render_node, first, last)
        
        print(f"  ✅ Render completed successfully for frames {first}-{last}.")
        
        # Important: Rendering does not modify the Nuke script file, so return False.
        if render_group_created:
            return True  # Indicate that the script was modified by create_render_group
        else :  
            return False 
        
    except Exception as e:
        msg = f"Rendering of '" + render_node_name + "' failed: {e}"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
        return False


# MODE 4: Delete Nodes
#------------------------------
def delete_nodes(nk_path, nodes_to_delete):

    print("  --->  Running delete_nodes procedure ...")
    try:
        changed = False
        for node_name in nodes_to_delete:
            node = nuke.toNode(node_name)
            if node:
                nuke.delete(node)
                print(f"  ✅ Deleted node: {node_name}")
                if not changed:
                    changed = True
                    
            else:
                print(f"  ⚠️ Node not found, skipping: {node_name}")
                SKIPPED.append((nk_path, f"Node not found: {node_name}"))   

        if not changed:
            return False  # No changes made
        return True  # Changes were made
    
    except Exception as e:
        msg = f"delete_nodes failed: {e}"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
        return False


# MODE 5: Reset Root CX
#------------------------------
def reset_rootcx(nk_path):
    """
    Utility function to execute the 'cxallreset' knob on the Nuke root node.
    This is often used to reset specific custom controls or variables linked to the script.
    """
    print("  --->  Executing nuke.root().knob('cxallreset').execute() ...")
    try:
        if nuke.root().knob('cxallreset'):
            nuke.root().knob('cxallreset').execute()
            print("  ✔ 'cxallreset' executed successfully.")
            return True
        else:
            print("  ▶ Warning: 'cxallreset' knob not found on the root node.")
            SKIPPED.append((nk_path, "'cxallreset' knob not found on root"))
            return False
    except Exception as e:
        print(f"  ❌ Failed to execute 'cxallreset': {e}")
        return False



# MODE 6: Rename Nodes
#------------------------------
def rename_nodes(nk_path, rename_map):
    """
    Renames nodes based on rename_map = {oldName: newName}.
    Returns True only if something actually changes.
    """
    print("  ---> >> Running rename_nodes procedure ...")
    changed = False

    try:
        for old_name, new_name in rename_map.items():
            node = nuke.toNode(old_name)

            if not node:
                print(f"  ⚠️ Node not found: {old_name}")
                SKIPPED.append((nk_path, f"Node not found: {old_name}"))
                continue

            # Check if new_name already exists
            if nuke.toNode(new_name):
                msg = f"Target name already exists: {new_name}"
                print(f"  ❌ {msg}")
                FAILED.append((nk_path, msg))
                continue

            # Rename
            node.setName(new_name)
            print(f"  ✅ Renamed '{old_name}' → '{new_name}'")
            changed = True

        return changed

    except Exception as e:
        msg = f"rename_nodes failed: {e}"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
        return False



# ----------------------------------------------------
# 🔄 MAIN PROCESSING LOGIC
# ----------------------------------------------------

def process_nk_file(nk_path, index, total):

    global MODE 
    mode_name = MODE_DESCRIPTIONS.get(MODE, "UNKNOWN MODE")
    
    print("\n======================================================")
    print(f"[PROCESS # {index} of {total}] Starting: {nk_path}")
    print("======================================================")

    # 1. Clear Nuke Script State
    print("  ---> Resetting Nuke script state ...")
    nuke.scriptClear() # Resetting Nuke script state

    if not os.path.exists(nk_path):
        msg = "File not found"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
        return

    print()

    # 2. Load NK File
    print("  ---> Loading NK file ...")
    try:
        nuke.scriptOpen(nk_path) #verbose = 1
        print("  ✔ File loaded successfully.")
    except Exception as e:
        msg = f"Failed to open NK file: {e}"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
        return

    print()

    # 3. Run Procedure based on MODE
    changed = False
    
    if MODE == 1:
        initial_setup(nk_path)
    elif MODE == 2:
        changed = create_render_group(nk_path, replace_render_group=REPLACE_RENDER_GROUP, render_version= RENDER_VERSION)
    elif MODE == 3:
        changed = render(nk_path, render_version = RENDER_VERSION)
    elif MODE == 4:
        changed = delete_nodes(nk_path, NODES_TO_DELETE)
    elif MODE == 5:
        changed = reset_rootcx(nk_path)        
    elif MODE == 6:
        changed = rename_nodes(nk_path, RENAME_MAP)    
    else:
        print("Invalid mode")
        return
    
    print()

    # 4. Handle Save/Skip
    if not changed:
        print("  ▶ No changes detected/procedure was non-modifying → Skipping backup & save.")
        return

    # Backup & Save if changes occurred
    # Backup
    backup_original(nk_path)
    print()
    # Save NK
    print("  ///////////////////////////////////////////////////////////////////")
    print("  --->  👉 💾 Saving NK file ...")
    
    try:
        nuke.scriptSave(nk_path)
        print(f"  ✅ Saved successfully: {nk_path}")
    except Exception as e:
        msg = f"Failed to save NK file: {e}"
        print("  ❌", msg)
        FAILED.append((nk_path, msg))
    print("  ///////////////////////////////////////////////////////////////////")    
    print()


# ----------------------------------------------------
# 📊 ENTRY POINT AND SUMMARY
# ----------------------------------------------------

def print_summary(total_files_processed):
    """Prints a summary including success, failed, and skipped counts."""
    
    num_failed = len(FAILED)
    num_skipped = len(SKIPPED)
    num_success = total_files_processed - (num_failed + num_skipped)
    
    print("\n====================================")
    print("            SUMMARY")
    print("====================================\n")
    
    # NEW: Print overall counts
    print("📊 OVERALL RESULTS:")
    print(f" - **Total Files Checked:** {total_files_processed}")
    if num_success > 0 :
        print(f" - ✅ **Successful:** {num_success}")
    if num_failed > 0 :
        print(f" - ❌ **Failed:** {num_failed}")
    if num_skipped > 0 :
        print(f" - ⚠️ **Skipped:** {num_skipped}")
    print("\n--- DETAILS ---\n")

    if FAILED:
        print("❌ FAILED FILES:")
        for f, reason in FAILED:
            print(f" - {f}")
            print(f"      → {reason}")
        print()
    else:
        print("✔ No failures.\n")

    if SKIPPED:
        print("⏭ SKIPPED :")
        for f, reason in SKIPPED:
            # We only print the path for skipped files for brevity
            print(f" - {f}") 
        print()
    else:
        print("✔ No skipped files.\n")

    print("====================================\n")



# ----------------------------------------------------
# MAIN FUNCTION
# ----------------------------------------------------
def main():
    # Get Mode
    mode_description = MODE_DESCRIPTIONS.get(MODE, "UNKNOWN MODE")
    
    # Clear Screen
    def clear_screen():
        os.system('cls' if os.name == 'nt' else 'clear')
    clear_screen()


    # Print intuitive starting message
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print(f"MODE: {MODE} ( {mode_description} )")
    extra_msg = "    ❗❗❗  IMPORTANT: THIS IS REPLACE MODE  ❗❗❗"

    match MODE:
        case 2:
            print("----> Create RENDER GROUP ( " , ">>> REPLACE ! <<<" if REPLACE_RENDER_GROUP else "SKIP IF EXIST", " )")
            if REPLACE_RENDER_GROUP :
                print(extra_msg)
        case 3:
            print("----> Render " , "( >>> WILL CREATE NEW RENDER GROUP - REPLACE EXISTING ! <<< )" if REPLACE_RENDER_GROUP else "")
            if REPLACE_RENDER_GROUP :
                print(extra_msg)
        case 4:
            print("Delete nodes : ", NODES_TO_DELETE)
        case 6:
            print()
            print("----> Rename nodes :")
            for old,new in RENAME_MAP.items():
                print(f"       {old} → {new}")

    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")

    nk_files = read_file_list(FILE_LIST)

    if not nk_files:
        print("\n❌ No NK files found. Exiting.\n")
        return

    print("\n=== FILES TO PROCESS ===")
    number_of_files_to_show = MAX_FILE_LIST
    for i,f in enumerate(nk_files):
        if i < number_of_files_to_show: 
            print(" -", f)
        else:
            print(" ...")
            break

    print()
    print("-------------------------------- ")
    total = len(nk_files) 
    print("Total files to process:", total)
    print("-------------------------------- ")
    print()

    answer = input("Continue? (Y/N) : ").strip().lower()
    if answer not in ("", "y"):
        print("❌ Canceled by user.")
        sys.exit(0)

    print("✅ Continuing...\n")

    # PROCESS EACH FILE
    for i, nk in enumerate(nk_files, start=1):
        process_nk_file(nk, i, total)

    print("\n=== ALL DONE ===\n")

    # Pass the total count to the summary function
    print_summary(total)



if __name__ == "__main__":
    main()
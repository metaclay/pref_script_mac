...
def process_nk_file(nk_path, index, total):
    ...
    mod_status_start = nuke.root().modified()
    if DEBUG : print(f"debug => mod_status start : {mod_status_start} ")
    print()
    ...
    if MODE == 1:
        ...
        if DEBUG : print(f"DEBUG => changed : {changed}")
    ...
    #         print("  ▶ No changes detected/procedure was non-modifying → Skipping backup & save.")
    #         return
    mod_status_end = nuke.root().modified()
    if DEBUG : print(f"debug => mod_status end : {mod_status_end} ")

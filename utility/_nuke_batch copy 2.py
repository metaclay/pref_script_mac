...
def load_and_normalize(path):
    ...
    # Remove volatile lines Nuke always rewrites
    volatile_patterns = [
        r"^version .*$",
        r"^build .*$",
        r"^released .*$",
    ...
    # Trim whitespace so diff is based only on content
    return text.strip()
...
def process_nk_file(nk_path, index, total):
    ...
    original_text = load_and_normalize(nk_path)
    original_hash = file_hash(original_text)
    ...
    # if MODE == 1:
    #     initial_setup(nk_path) # just open and save - nodes clean up will be loaded automatically
    ...
    # elif MODE == 3:
    #     render(nk_path, render_version=RENDER_VERSION)
    ...
    if os.path.exists(tmp_out):
        os.remove(tmp_out)
    nuke.scriptSaveAs(tmp_out)

    new_text = load_and_normalize(tmp_out)
    new_hash = file_hash(new_text)
    os.remove(tmp_out)
